...
class UnMaya_Alembic_Exporter():
    ...
    def exporter(self):
        ...
        anim_filepath = self.SHOTS_DIR + "/" + self.shot_selection + "/" + self.ANIM_DIR
        if not self.dir_exists(anim_filepath):
            os.mkdir(self.alem_filepath)

        self.alem_filepath = self.SHOTS_DIR + "/" + self.shot_selection + "/" + self.ANIM_DIR + "/" + self.object_selection
        print(self.alem_filepath)
        
        if not self.dir_exists(self.alem_filepath):
            print("file does not exist")
            os.mkdir(self.alem_filepath)
        
        command = self.get_alembic_command(self.alem_filepath)
    ...
    def get_alembic_command(self, file_path):
        ...
        for obj in curr_selection:
            if root != "":
                root = root + " "
            root = root + "-root " + obj
        save_name = file_path + "/" + self.object_selection + "_main.abc"
    ...
    def get_frameRange_gui(self):
        ...
        cmds.button(label='Next', command=lambda x: self.get_frameRange())
        cmds.setParent('..')
        cmds.showWindow(self.window)
