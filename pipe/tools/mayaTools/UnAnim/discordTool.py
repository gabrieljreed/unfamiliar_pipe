...
def load_webhooks():
    ...
    if os.path.exists(webhooks_file):
        with open(webhooks_file, 'r') as f:
            webhooks = loads(f.read())["webhooks"]
    else:
...
def set_persistent_settings_maya_to_discord(custom_username, webhook, image_format, video_format, video_scale,
                                            video_compression, video_output_type):
    """
    ...
    Args:
        ...
        video_output_type (str): One of these three strings 'qt', 'avi', or 'movie' as
                                 determined by the cmds.playblast function
...
def build_gui_maya_to_discord():
    ...
    # Build UI
    window_gui_maya_to_discord = cmds.window(window_name, title=' ' + script_name + ' - (v' + script_version + ')',
    ...
    cmds.rowColumnLayout(nc=4, cw=[(1, 10), (2, 150), (3, 60), (4, 40)], cs=[(1, 10), (2, 0), (3, 0)],
    ...
    if os.path.isdir(icons_folder_dir) and os.path.exists(icon_image) is False:
    ...
    cmds.rowColumnLayout(nc=3, cw=[(1, 100), (2, 50), (3, 100), (4, 50)], cs=[(1, 10), (2, 0), (3, 0), (4, 0)],
                         p=content_main)
    ...
    if gt_mtod_settings['is_first_time_running'] is True:
        cmds.text(webhook_name_text, e=True, l='Set Webhook in the Settings', bgc=[1, 1, 0])
    else:
        if 'Error' in gt_mtod_settings.get('discord_webhook_name') or 'Missing Webhook' in gt_mtod_settings.get(
                'discord_webhook_name'):
    ...
    cmds.rowColumnLayout(nc=1, cw=[(1, 260), (2, 1), (3, 5)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main)
    send_message_btn = cmds.iconTextButton(style='iconAndTextHorizontal', image1=send_message_btn_ico,
                                           label='Send Message Only',
                                           statusBarMessage='This button will record a playblast'
                                                            ' and attempt to send it to Discord.',
                                           olc=[1, 0, 0], enableBackground=True, bgc=[.3, .3, .3], h=40,
                                           marginWidth=60,
    ...
    cmds.rowColumnLayout(nc=1, cw=[(1, 260), (2, 1), (3, 5)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main)
    send_desktop_btn = cmds.iconTextButton(style='iconAndTextVertical', image1=send_desktop_btn_ico,
                                           label='Send Desktop Screenshot',
                                           statusBarMessage='This button will take a screenshot of the entire desktop'
                                                            ' and send it to Discord. (In case of multiple monitors,'
                                                            ' it will use the one with the main Maya window)',
                                           olc=[1, 0, 0], enableBackground=True, bgc=screenshot_btn_color, h=80,
                                           command=lambda: send_desktop_screenshot())

    cmds.separator(h=2, style='none')  # Empty Space

    cmds.rowColumnLayout(nc=2, cw=[(1, 128), (2, 128), (3, 5)], cs=[(1, 10), (2, 4), (3, 0)], p=content_main)
    send_maya_window_btn = cmds.iconTextButton(style='iconAndTextVertical', image1=send_maya_window_btn_ico,
                                               label='Send Maya Window',
                                               statusBarMessage='This button will take a screenshot of Maya window'
                                                                ' without any other elements that might be on top'
                                                                ' of it, then send it to Discord',
                                               olc=[1, 0, 0], enableBackground=True, bgc=screenshot_btn_color, h=80,
                                               command=lambda: send_maya_window())

    send_viewport_btn = cmds.iconTextButton(style='iconAndTextVertical', image1='hypershadeOutlinerPerspLayout.png',
                                            label='Send Viewport',
                                            statusBarMessage='This button will take a screenshot of the currently '
                                                             'active viewport then send it to Discord.',
                                            olc=[1, 0, 0], enableBackground=True, bgc=screenshot_btn_color, h=80,
    ...
    cmds.separator(h=5, style='none')  # Empty Space
    cmds.rowColumnLayout(nc=1, cw=[(1, 260), (2, 1), (3, 5)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main)
    send_playblast_btn = cmds.iconTextButton(style='iconAndTextVertical', image1=send_playblast_btn_ico,
                                             label='Send Playblast',
                                             statusBarMessage='This button will record a playblast and attempt to '
                                                              'send it to Discord.',
    ...
    send_obj_btn = cmds.iconTextButton(style='iconAndTextVertical', image1=send_obj_btn_ico, label='Send OBJ',
                                       statusBarMessage='This button will export your selection as OBJ and attempt '
                                                        'to send it to Discord. (See "Help" for more information)',
                                       olc=[1, 0, 0], enableBackground=True, bgc=objects_btn_color, h=80,
    ...
    send_fbx_btn = cmds.iconTextButton(style='iconAndTextVertical', image1=send_fbx_btn_ico, label='Send FBX',
                                       statusBarMessage='This button will export your selection as FBX and attempt '
                                                        'to send it to Discord. (See "Help" for more information)',
                                       olc=[1, 0, 0], enableBackground=True, bgc=objects_btn_color, h=80,
    ...
    def get_username():
        ...
        A custom username may be used, in which case the function returns the custom username followed by
    ...
    def clear_attached_message(response):
        ...
        Args:
            response (dict): A dictionary response received from an HTTP object after post/get operation.
    ...
    def attached_text_message(operation_name, response):
        ...
        Args:
            ...
            response (dict): A dictionary response received from an HTTP object after post/get operation.
                             (This should be the response of the previous operation)
            ...
            if status_value in success_codes:
                try:
                    ...
                    if upload_message.strip() != '':
                        ...
                        response_inview_feedback(operation_name, response,
                                                 display_inview=gt_mtod_settings.get('feedback_visibility'))
                except Exception as e:
                    logger.debug(str(e))

    def disable_buttons():
        """ Disable buttons so user don't accidentally send multiple requests at once """
        ...
        if gt_mtod_settings.get('is_webhook_valid'):
            ...
            if len(selection) > 0:
                try:
                    ...
                    temp_exported_obj = cmds.file(temp_path, pr=1, typ="OBJexport", es=1, f=True,
    ...
    cmds.showWindow(window_gui_maya_to_discord)
...
def build_gui_help_maya_to_discord():
    ...
    cmds.text(l='Copy the "Webhook URL" and load it in the settings for this script.', align="center", font=help_font)
    cmds.separator(h=7, style='none')  # Empty Space
...
def build_gui_settings_maya_to_discord():
    ...
    new_username_input = cmds.textField(pht='username (not required)', text=current_custom_username,
                                        font='smallPlainLabelFont')
    ...
    video_scale_input = cmds.intSliderGrp(field=True, minValue=1, maxValue=100, fieldMinValue=1, fieldMaxValue=100,
                                          value=current_video_scale, cw=([1, 35], [2, 65]))
    ...
    feedback_visibility_chk = cmds.checkBox(label='', value=gt_mtod_settings.get('feedback_visibility'),
                                            cc=lambda args: update_checkbox_settings_data())
    cmds.text(l='Display Viewport Feedback', align="left", font='smallPlainLabelFont')
    timestamp_visibility_chk = cmds.checkBox(label='', value=gt_mtod_settings.get('timestamp_visibility'),
                                             cc=lambda args: update_checkbox_settings_data())
    ...
    def update_available_compressions():
        ...
        try:
            ...
            for option in get_available_playblast_compressions(cmds.optionMenu(output_type_input, q=True, value=True)):
                cmds.menuItem(label=option, p=compression_input)
        except Exception as exception:
            logger.debug(str(exception))
    ...
    def reset_settings():
        """
        Resets fields in the settings (do not affect stored variables or persistent settings)
        ...
        for index, obj_out in enumerate(cmds.optionMenu(output_type_input, q=True, itemListLong=True)):
            if cmds.menuItem(obj_out, q=True, label=True) == gt_mtod_settings_default.get('video_output_type'):
                cmds.optionMenu(output_type_input, e=True, select=index + 1)

        update_available_compressions()

        found_default = False
        for index, obj_out in enumerate(cmds.optionMenu(compression_input, q=True, itemListLong=True)):
            if cmds.menuItem(obj_out, q=True, label=True) == gt_mtod_settings_default.get('video_compression'):
                cmds.optionMenu(compression_input, e=True, select=index + 1)
                found_default = True
    ...
    def apply_settings():
        ...
        set_persistent_settings_maya_to_discord(cmds.textField(new_username_input, q=True, text=True),
                                                cmds.textField(new_webhook_input, q=True, text=True),
                                                cmds.textField(new_image_format_input, q=True, text=True),
                                                cmds.textField(new_video_format_input, q=True, text=True),
                                                cmds.intSliderGrp(video_scale_input, q=True, value=True),
                                                cmds.optionMenu(compression_input, q=True, value=True),
...
def capture_desktop_screenshot(image_file):
    ...
    frame = app.primaryScreen().grabWindow(0, screen_geometry.x(), screen_geometry.y(), screen_geometry.width(),
...
def capture_playblast_animation(video_file, scale_pct, compression, video_format):
    ...
    message = '<span style=\"color:#FFFFFF;\">Playblast File Size:' \
              ...
              get_readable_size(file_size, precision=2) + '</span>'
    if gt_mtod_settings.get('feedback_visibility'):
        cmds.inViewMessage(amg=message, pos='botLeft', fade=True, alpha=.9)
        cmds.inViewMessage(amg='<span style=\"color:#FFFFFF;\">Open the the script editor for more information',
...
def discord_get_webhook_name(webhook_url):
    ...
    Returns:
        name
    (str): The name of the webhook (or error string, if operation failed)
    """
    success_codes = [200, 201, 202, 203, 204, 205, 206]
    if python_version == 3:
        try:
            ...
            print(e)
            cmds.warning(
                'Error connecting to the provided webhook. '
                'Make sure you\'re pasting the correct URL and check your internet connection or firewall.')
...
def get_readable_size(size, precision=2):
    """
    Returns a human-readable version of the size of a file
    ...
    Returns:
        formatted_string (string) : Size + Suffix
...
def response_inview_feedback(operation_name, response, write_output=True, display_inview=True):
    ...
    Args:
        ...
        response (dict): A dictionary response received from an HTTP object after post/get operation.
        write_output (bool): Determines if the functions will write an extra output text
                             (Like a "Result: pCube1" text output)
        display_inview (bool): Determines if generated message will be displayed as an inView message (visibility)
    """
    ...
    if len(response) >= 1:
        ...
        if status_value in success_codes:
            message += 'The ' + str(operation_name) + \
                       ' was <span style=\"color:#00FF00;text-decoration:underline;\">sent successfully</span>.'
            if write_output:
                sys.stdout.write('The ' + str(operation_name) + ' was sent successfully.  Web response: ' + str(
                    reason_value) + ' (' + str(status_value) + ')')
        else:  # Error
            message += 'The ' + str(
                operation_name) + ' was <span style=\"color:#FF0000;text-decoration:underline;\">not sent.'
            if write_output:
                sys.stdout.write(
                    'The ' + str(operation_name) + ' was sent.  Web response: ' + str(reason_value) + ' (' +
                    str(status_value) + ')')
    else:
        message += 'The ' + str(operation_name) + \
                   ' was <span style=\"color:#FF0000;text-decoration:underline;\">not sent.'
        if write_output:
            sys.stdout.write('The ' + str(operation_name) + ' was not sent.  Error: Web response can\'t be read.')
