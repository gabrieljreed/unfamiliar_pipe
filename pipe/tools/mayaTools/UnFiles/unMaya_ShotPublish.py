...
class UnMaya_ShotPublish:
    ...
    # Checks to see if shot is checked out to the current user. Gives option to check out if it is not currently, or
    # blocks publishing if the current user is not the current owner
    def checkout_check(self):
        ...
        if assigned_user == "":
            confirm = cmds.confirmDialog(title='WARNING',
                                         message="This shot is not currently checked out. Check it out now?",
                                         button=['Yes', 'No'], defaultButton='Yes', dismissString='Other')
            if confirm == "Yes":
                print("Checking out shot")
                curr_user = self.curr_env.get_username()
                temp_el.assign_user(curr_user)
                temp_el.write_element_file()
                confirm = cmds.confirmDialog(title='Complete',
                                             message="The shot is now checked out to you. Continue with publishing?",
                                             button=['Yes', 'No'], defaultButton='Yes', dismissString='Other')
                if confirm == "Yes":
                    self.select_shot_gui()
                else:
            ...
            confirm = cmds.confirmDialog(title='WARNING',
                                         message="This shot is checked out to someone else. Close shot?",
                                         button=['Yes', 'No'], defaultButton='Yes', dismissString='Other')
            if confirm == "Yes":
    ...
    def select_shot_gui(self):
        ...
        cmds.button(label="Publish Shot", backgroundColor=[0, 1, 1],
                    c=lambda x: self.publish(self.getSelected(selection)))
        cmds.setParent("..")
    ...
    # Allows user to check in the current shot without publishing.
    # Reverts the shot file to the most recent version to erase any changes since last publish
    def checkin_without_saving(self):
        confirm = cmds.confirmDialog(
            title='WARNING',
            message="Progress since last publish will not be saved. Would you still like to continue?",
            button=['Yes', 'No'], defaultButton='Ok', dismissString='Other')
    ...
    def quit_shot(self, type):
        if type == "CHECKIN":
            confirm = cmds.confirmDialog(
                title='Shot Checked in',
                message="Shot successfully checked-in or returned. Close shot and keep Maya open, or exit Maya?",
                button=['Close Shot', 'Exit Maya'], defaultButton='Exit Maya',
                dismissString='Other')
            ...
            confirm = cmds.confirmDialog(
                ...
                message="Shot successfully published. Keep working, close shot but keep Maya open, or exit Maya?",
                button=['Continue Working', 'Close Shot', 'Exit Maya'], defaultButton='Exit Maya',
                dismissString='Other')
